...
# python VIDEO_PARTIAL_LOAD_TEST_CUT_V2.py
...
def section_image_prediction(file, interval, full_face_data,full_face_label):
    ...
    while samples_wrote < samples_total:
        ...
        if len(block) >= int(interval * fps):
            block_probability = facial_model.predict_proba(block)
    ...
    for emotion in mlp_emotions:   
        ...
        for d_index,d_item in enumerate(dataset_directory):
            ...
            for file in glob.glob(d_item %emotion): 
                ...
                face_probability = section_image_prediction(file, interval, full_face_data,full_face_label)
